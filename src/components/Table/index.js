...
export default {
  ...
  props: Object.assign({}, T.props, {
    ...
    resultProcess: { // 处理查询结果，可以返回新结果或修改结果的内容
      type: Function,
      required: false
    },
    method: {
  ...
  methods: {
    ...
    search (params, paramCallback) {
      ...
      if (paramCallback) {
        ...
        const b = paramCallback(parameter)
        if (b) {
          parameter = b
        }
      }
      ...
      let result = null
      if (this.method === 'get') {
        result = this.$http.get(this.url, { params: parameter })
                      .then(resp => resp.result)
      } else {
        result = this.$http.post(this.url, parameter)
                      .then(resp => resp.result)
      }
      this.processResult(null, result)
    },
    /**
     * 加载数据方法
    ...
    loadData (pagination, filters, sorter) {
      ...
      this.processResult(pagination, result)
    },
    processResult (pagination, result) {
      // 对接自己的通用数据接口需要修改下方代码中的 r.pageNum, r.totalCount, r.data
      // eslint-disable-next-line
      if ((typeof result === 'object' || typeof result === 'function') && typeof result.then === 'function') {
        result.then(r => {
          if (this.resultProcess) {
            const newR = this.resultProcess(r)
            if (newR) {
              r = newR
            }
          }
