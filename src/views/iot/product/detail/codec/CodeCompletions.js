...
var variables = 'Array|Boolean|Date|Function|Iterator|Number|Object|RegExp|String|' + // Constructors
...
var keys = 'const|yield|import|get|set|' +
...
function _getCompletions (type) {
  ...
  if (type === 'MODBUS') {
    list = [
      ...
      // session
      { caption: 'session.ReadDiscreteInputs()', meta: 'session', value: 'var resp = session.ReadDiscreteInputs(startingAddress, length)' },
      { caption: 'session.ReadCoils()', meta: 'session', value: 'var resp = session.ReadCoils(startingAddress, length)' },
      { caption: 'session.ReadInputRegisters()', meta: 'session', value: 'var resp = session.ReadInputRegisters(startingAddress, length)' },
      { caption: 'session.ReadHoldingRegisters()', meta: 'session', value: 'var resp = session.ReadHoldingRegisters(startingAddress, length)' },
    ...
    list = [
      { caption: 'context', meta: 'common', value: 'context', remark: '上下文，可以获取到消息、session、设备等数据' },
      // OnMessage
      { caption: 'context.GetMessage()', meta: 'OnMessage', value: 'var message = context.GetMessage()', remark: '获取消息信息' },
      { caption: 'context.MsgToString()', meta: 'OnMessage', value: 'var str = context.MsgToString()', remark: '返回消息的字符串格式' },
      ...
      { caption: 'context.SaveEvents()', meta: 'OnMessage', value: 'context.SaveEvents("eventId", {"key":"value"})', remark: '保存物模型事件数据' },
      { caption: 'context.ReplyOk()', meta: 'OnMessage', value: 'context.ReplyOk()', remark: '指令下发成功' },
      { caption: 'context.ReplyFail()', meta: 'OnMessage', value: 'context.ReplyFail("resaon")', remark: '指令失败' },
      // deviceOper
      { caption: 'deviceOper.GetConfig()', meta: 'deviceOper', value: 'var value = deviceOpr.GetConfig("key")', remark: '获取设备配置' },
      { caption: 'deviceOper.GetData()()', meta: 'deviceOper', value: 'var map = deviceOpr.GetData()()' },
      // session
      { caption: 'session.Disconnect()', meta: 'session', value: 'session.Disconnect()', remark: '断开连接' },
      // OnInvoke
      { caption: 'context.GetMessage()', meta: 'OnInvoke', value: 'var message = context.GetMessage()', remark: '获取下发信息' },
      { caption: 'context.GetDevice()', meta: 'OnInvoke', value: 'var deviceOper = context.GetDevice()' },
      { caption: 'context.ReplyOk()', meta: 'OnInvoke', value: 'context.ReplyOk()', remark: '指令下发成功' },
      { caption: 'context.ReplyFail()', meta: 'OnInvoke', value: 'context.ReplyFail("resaon")', remark: '指令失败' },
      // FuncInvoke
      { caption: 'message.FunctionId', meta: 'FuncInvoke', value: 'var functionId = message.FunctionId;', remark: '物模型功能id' },
      { caption: 'message.Data', meta: 'FuncInvoke', value: 'var data = message.Data;', remark: '下发的数据' }
    ]
    if (type === 'MQTT_BROKER' || type === 'MQTT_CLIENT') {
      // OnConnect
      list = list.concat([
        { caption: 'context.GetClientId()', meta: 'OnConnect', value: 'var clientId = context.GetClientId()', remark: '获取Mqtt的clentId' },
      ...
      list = list.concat([
        { caption: 'session.Send()', meta: 'session', value: 'session.Send("text")', remark: '发送文本数据' },
        { caption: 'session.SendHex()', meta: 'session', value: 'session.SendHex("68657820737472696e67")', remark: '发送二进制数据, 内容为16进制字符串, 会转成byte数组' }
      ])
    }
  }
  return list
