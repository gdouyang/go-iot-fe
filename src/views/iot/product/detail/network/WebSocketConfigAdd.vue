<template>
  <div>
    <Dialog ref="addModal" title="WebSocket配置" @confirm="addConfirm" @close="addClose">
      <a-form-model
        ref="addFormRef"
        :model="addObj"
        style="width: 90%"
  ...
  methods: {
    open (productId) {
      ...
      .then(data => {
        ...
        const defaultValue = _.cloneDeep(defaultWebSocketAddObj)
      ...
      this.$refs.addFormRef.validate((valid) => {
        if (valid) {
          ...
          _.forEach(saveData.configuration.routers, r => {
            r.protocol = 'websocket-server-script-protocol'
