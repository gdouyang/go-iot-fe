export default function encodeQueryParam (params) {
    ...
    if (key === 'terms') {
      ...
      Object.keys(terms).forEach((k) => {
        if (!(terms[k] === '' || terms[k] === undefined || terms[k].length === 0 || terms[k] === {})) {
          ...
          if (!queryParam.terms[index]) {
            queryParam.terms[index] = {}
          }
          if (k.indexOf('@') > -1) {
            const temp = k.split('@')
      ...
      if (Object.keys(sorts).length > 0) {
        queryParam.sorts.push({
          name: sorts.field,
          order: (sorts.order || '').replace('end', '')
        })
      }
    } else {
  ...
  //   pp[encodeURIComponent(key)] = queryParam[key]
  // }
  return queryParam
}
...
export function encodeQueryParamGet (params) {
  if (!params) return {}
  const queryParam = {}
  // 格式化查询参数
  const { terms } = params
